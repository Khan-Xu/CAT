...
for islit in range(8):
    ...
    # # test one coherent mode
    scr._svd()
    scr.n = 6
    # scr.cal_i()
    
    # CF.append(scr.ratio[0] / np.sum(scr.ratio))
    
    flux_ratio.append(
        100 * np.sum(scr.intensity * scr.xpixel * scr.ypixel) / 
        np.sum(sr0.intensity * sr0.xpixel * sr0.ypixel)
        )
    # intensitys.append(np.abs(scr.cmode[0]))
