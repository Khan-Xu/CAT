...
if __name__ == '__main__':
    ...
    ssa, sc0, sc1, sc2 = srw_cat_compare_propagators()
    ...
    plt.pcolor(ssa.xtick*1e6, ssa.xtick*1e6, np.abs(ssa.cmode[0])**2)
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    # focus
    
    plt.figure(figsize = (6, 6))
    plt.pcolor(srw_xtick*1e3, srw_ytick*1e3, intensity)
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    ...
    plt.plot(srw_xtick*1e3, np.sum(intensity, 0) / np.max(np.sum(intensity, 0)))
    plt.plot(sc0.xtick*1e6 + 0.800, np.sum(np.abs(sc0.cmode[0])**2, 0) / np.max(np.sum(np.abs(sc0.cmode[0])**2, 0)))
    plt.plot(sc0.xtick*1e6 + 2.089, np.sum(np.abs(sc1.cmode[0])**2, 0) / np.max(np.sum(np.abs(sc1.cmode[0])**2, 0)))
    plt.plot(sc0.xtick*1e6 + 1.709, np.sum(np.abs(sc2.cmode[0])**2, 0) / np.max(np.sum(np.abs(sc2.cmode[0])**2, 0)))
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)